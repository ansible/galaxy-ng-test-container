...
def publish_collection(module, collection):
    ...
    result = {
        'collection': "%s-%s-%s" % (namespace, name, version),
    }
    collection_dir = os.path.join(output_dir, "%s-%s-%s" % (namespace, name, version))
    ...
    with tempfile.NamedTemporaryFile(mode='wb') as temp_fd:
        ...
        if module.params['output_dir'] is not None:
            ...
            signature_path = os.path.join(module.params['output_dir'], '%s-%s-%s-MANIFEST.json.asc' % (namespace, name, version))
